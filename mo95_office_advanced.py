...
class MO95Office:
    ...
    def on_ruler_drag(self, event):
        ...
        self.status_label.config(text=f"Margins: Left={self.left_margin:.2f}\" Right={self.right_margin:.2f}\"")
        
    def on_ruler_release(self, event):
        """Handle releasing margin drag"""
        self.dragging_margin = None
        
    def apply_margins_to_document(self):
        """Apply current margins to the entire document"""
        try:
            # Calculate margin pixels (96 DPI)
            left_pixels = int(self.left_margin * 96)
            right_pixels = int(self.right_margin * 96)
            
            # Configure text widget margins
            self.text_editor.configure(padx=left_pixels)
            
            # Note: Right margin is harder to enforce in Tkinter Text widget
            # This provides visual guidance via the ruler
            
        except Exception as e:
            pass
        
    def create_editor(self):
        editor_container = tk.Frame(self.root, bg=self.gray_dark)
        editor_container.pack(side=tk.TOP, fill=tk.BOTH, expand=True, padx=4, pady=4)
        
        scrollbar = tk.Scrollbar(editor_container, orient=tk.VERTICAL)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        # Calculate initial padx from left margin
        initial_padx = int(self.left_margin * 96)
        
        self.text_editor = tk.Text(editor_container, wrap=tk.WORD, undo=True, 
                                   font=(self.current_font_family, self.current_font_size), 
                                   bg=self.white, fg="black", 
                                   yscrollcommand=scrollbar.set,
                                   relief=tk.FLAT, bd=0, padx=initial_padx, pady=15,
    ...
    def show_about(self):
        ...
        tk.Label(content_frame, text="With ASCII Tables & Fixed Margins", bg=self.white, 
